...
const testCases: BridgeTest[] = [
  ...
  // === Unit suffixes ===
  {
    ...
    expectedText: "1s 500ms",
    expectedHint: "LONG_WAIT_MICROSECONDS",
    description: "MICROSECONDS suffix",
  },
  {
    token: "750",
    ...
    description: "NANOSECONDS suffix",
  },
  ...
  // === Keyword detection ===
  {
    ...
    description: "TTL keyword → seconds",
  },
  {
    token: "86400000",
    ...
    description: "CACHE_EXPIRY keyword",
  },
  {
    token: "5000",
    ...
    description: "RETRY_DELAY_MS keyword",
  },
  ...
  // === Function call patterns (with language) ===
  {
    ...
    description: "setTimeout in JavaScript → milliseconds",
  },
  {
    token: "5000",
    line: "setInterval(callback, 5000)",
    language: "javascript",
    expectedText: "5s",
    expectedHint: "setInterval",
    description: "setInterval in JavaScript → milliseconds",
  },
  {
    token: "2000",
    line: "requestAnimationFrame(fn, 2000)",
    language: "javascript",
    expectedText: "2s",
    expectedHint: "requestAnimationFrame",
    description: "requestAnimationFrame in JavaScript → milliseconds",
  },
  {
    token: "2.5",
    line: "time.sleep(2.5)",
    language: "python",
    expectedText: "2s 500ms",
    expectedHint: "time.sleep",
    description: "time.sleep in Python → seconds",
  },
  {
    token: "1000000000",
    line: "time.Sleep(1000000000)",
    language: "go",
    expectedText: "1s",
    expectedHint: "time.Sleep",
    description: "time.Sleep in Go → nanoseconds",
  },
  {
    token: "10.0",
    line: "timeout = 10.0",
    ...
    description: "Simple timeout variable",
  },

  // === HTTP client patterns ===
  {
    token: "30",
    line: "client = httpx.Client(timeout=30)",
    expectedText: "30s",
    expectedHint: "timeout",
    description: "httpx.Client timeout kwarg",
  },
  {
    token: "3600",
    line: "cache.set(key, value, ttl=3600)",
    expectedText: "1h",
    expectedHint: "ttl",
  ...
  // === Ignored patterns (should return null) ===
  {
    ...
    description: "Hex color ignored",
  },
  {
    token: "1700000000",
    line: "UNIX_TIMESTAMP = 1700000000",
    description: "Unix epoch ignored (timestamp in line)",
  },
  ...
  { token: "0", line: "ZERO = 0", description: "Zero ignored" },
  {
    ...
    description: "Value above maxValue ignored",
  },
];
...
describe("TimeLens Neovim Bridge", () => {
  ...
  describe("Input validation", () => {
    it("handles empty token", async () => {
      const response = await runBridge(
        JSON.stringify({ token: "", line: "test" }),
      );
      expect(response).toBeNull();
    });
  ...
  describe("Format consistency", () => {
    it("returns compact format by default", async () => {
      const response = await runBridge(
        JSON.stringify({ token: "3600", line: "timeout = 3600" }),
      );
      expect(response).not.toBeNull();
      if (response) {
  ...
  describe("Hint generation", () => {
    it("includes keyword hint for timeout", async () => {
      const response = await runBridge(
        JSON.stringify({ token: "30", line: "timeout = 30" }),
      );
      expect(response).not.toBeNull();
      if (response) {
    ...
    it("includes keyword hint for retry", async () => {
      const response = await runBridge(
        JSON.stringify({ token: "5000", line: "retryDelay = 5000" }),
      );
      expect(response).not.toBeNull();
      if (response) {
    ...
    it("includes keyword hint for ttl", async () => {
      const response = await runBridge(
        JSON.stringify({ token: "86400", line: "ttl = 86400" }),
      );
      expect(response).not.toBeNull();
      if (response) {
    ...
    it("does not include hint when none detected", async () => {
      const response = await runBridge(
        JSON.stringify({ token: "500", line: "value = 500" }),
      );
      expect(response).not.toBeNull();
      if (response) {
